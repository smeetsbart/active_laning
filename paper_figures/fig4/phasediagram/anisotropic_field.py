...
f_n       = Variable( "f_n"       , params, value = 3.0  * u("dimensionless") )#Self-reinforcement rate (1/reinforcement time)
...
out_int    = Variable( "out_int"  , params, value = 2*60. * u("min"))
endtime    = Variable( "endtime"  , params, value = 100.0 * u('hour') )
...
a          = VariableFunction("a"        , params, function='$f_n$*$D$/$F_cell$**2')
k          = VariableFunction("k"        , params, function='$a$*$gamma_s$')
...
#-----------------------------------------------------------------------------------------------------------------------
#Only save the last hour
#mysim.run_until( endtime.get_value() - 1*u('hour') )
